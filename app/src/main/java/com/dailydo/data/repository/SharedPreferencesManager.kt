...
class SharedPreferencesManager(context: Context) {
    ...
    companion object {
        ...
        private const val KEY_LAST_MOOD_SYNC_TIMESTAMP = "last_mood_sync_timestamp"
        ...
        private const val KEY_DAILY_SUMMARY_COUNT = "daily_summary_count"
        private const val KEY_DAILY_SUMMARY_PREFIX = "daily_summary"
    ...
    fun getHabitProgressForHabit(habitId: String): List<HabitProgress> {
        return getHabitProgress().filter { it.habitId == habitId }
    }
    ...
    fun updateMoodEntry(entry: MoodEntry): MoodEntry {
        saveMoodEntry(entry)
        return entry
    }
    ...
    fun getLastMoodSyncTimestamp(): Long {
        return sharedPrefs.getLong(KEY_LAST_MOOD_SYNC_TIMESTAMP, 0L)
    }
    
    fun setLastMoodSyncTimestamp(timestamp: Long) {
        sharedPrefs.edit().putLong(KEY_LAST_MOOD_SYNC_TIMESTAMP, timestamp).apply()
    }
    ...
    fun getHydrationData(): List<DailyHydration> {
        val intakeList = getHydrationIntake()
        val settings = getHydrationSettings()
        
        // Group intake by date
        val intakeByDate = intakeList.groupBy { it.date }
        
        // Create DailyHydration objects for each date
        return intakeByDate.map { (date, intakes) ->
            val totalIntake = intakes.sumOf { it.amountMl }
            val goalReached = totalIntake >= settings.dailyGoalMl
            val goalReachedTime = if (goalReached) {
                intakes.sortedBy { it.timestamp }.firstOrNull { it.amountMl > 0 }?.timestamp
            } else null
            ...
            DailyHydration(
                ...
                goalReachedTime = goalReachedTime
            )
        }.sortedByDescending { it.date }
    }

    // DAILY SUMMARY MANAGEMENT
    fun saveDailySummaries(summaries: List<DailySummary>) {
        val editor = sharedPrefs.edit()
        val existing = sharedPrefs.getInt(KEY_DAILY_SUMMARY_COUNT, 0)
        editor.remove(KEY_DAILY_SUMMARY_COUNT)
        for (i in 0 until existing) {
            val p = "${KEY_DAILY_SUMMARY_PREFIX}_$i"
            editor.remove("${p}_date")
            editor.remove("${p}_habits")
            editor.remove("${p}_mood")
            editor.remove("${p}_hydration")
            editor.remove("${p}_generatedAt")
        }
        editor.putInt(KEY_DAILY_SUMMARY_COUNT, summaries.size)
        summaries.forEachIndexed { index, s ->
            val p = "${KEY_DAILY_SUMMARY_PREFIX}_$index"
            editor.putString("${p}_date", s.date)
            editor.putFloat("${p}_habits", s.habitsPercent)
            editor.putFloat("${p}_mood", s.moodPercent)
            editor.putFloat("${p}_hydration", s.hydrationPercent)
            editor.putLong("${p}_generatedAt", s.generatedAt.time)
        }
        editor.apply()
    }

    fun getDailySummaries(): List<DailySummary> {
        val count = sharedPrefs.getInt(KEY_DAILY_SUMMARY_COUNT, 0)
        val list = mutableListOf<DailySummary>()
        for (i in 0 until count) {
            val p = "${KEY_DAILY_SUMMARY_PREFIX}_$i"
            val date = sharedPrefs.getString("${p}_date", null) ?: continue
            val habits = sharedPrefs.getFloat("${p}_habits", 0f)
            val mood = sharedPrefs.getFloat("${p}_mood", 0f)
            val hydration = sharedPrefs.getFloat("${p}_hydration", 0f)
            val gen = java.util.Date(sharedPrefs.getLong("${p}_generatedAt", System.currentTimeMillis()))
            list.add(DailySummary(date, habits, mood, hydration, gen))
        }
        return list
    }

    fun saveDailySummary(summary: DailySummary) {
        val all = getDailySummaries().toMutableList()
        val idx = all.indexOfFirst { it.date == summary.date }
        if (idx >= 0) all[idx] = summary else all.add(summary)
        saveDailySummaries(all)
    }

    fun getSummaryForDate(date: String): DailySummary? {
        return getDailySummaries().find { it.date == date }
    }
    ...
    fun calculateOverallHabitStreak(): Int {
        val allProgress = getHabitProgress()
        ...
        for (i in 0 until 365) {
            val dateString = dateFormat.format(calendar.time)
            val hasAnyCompleted = allProgress.containsKey(dateString)
            if (hasAnyCompleted) {
                streak++
            } else {
                break
    ...
    // Calculate all-habits completion streak: consecutive days where all ACTIVE habits were completed
    fun calculateAllHabitsCompletionStreak(): Int {
        val activeHabits = getHabits().filter { it.isActive }
        if (activeHabits.isEmpty()) return 0

        val progressByDate = getHabitProgress().groupBy { it.date }
        var streak = 0
        val calendar = Calendar.getInstance()
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        ...
        for (i in 0 until 365) {
            val dateString = dateFormat.format(calendar.time)
            val day = progressByDate[dateString].orEmpty()
            val allCompleted = activeHabits.all { habit ->
                day.any { it.habitId == habit.id && it.isCompleted }
            }
            if (allCompleted) {
                streak++
            } else {
