...
import com.dailydo.ui.adapters.HabitsAdapter
...
class HabitsFragment : Fragment() {
    ...
    private lateinit var tvHabitTotalCount: TextView
    ...
    private fun initializeViews(view: View) {
        ...
        tvStreakValue = view.findViewById(R.id.tv_streak_value)
        tvHabitTotalCount = view.findViewById(R.id.tv_habit_total_count)
    }
    ...
    private fun loadHabits() {
        ...
        // Update quick card total count
        tvHabitTotalCount.text = habits.size.toString()

        // Update all-habits completion streak (100% completed days in a row)
        val allHabitsStreak = prefsManager.calculateAllHabitsCompletionStreak()
        tvStreakValue.text = if (allHabitsStreak == 1) "1 day" else "$allHabitsStreak days"
        ...
        // Avoid updating adapter while RecyclerView is computing layout
        recyclerView.post {
    ...
    private fun showAddHabitDialog() {
        ...
        // Set to no selection (empty state)
        ...
        androidx.appcompat.app.AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(getString(R.string.save)) { _, _ ->
                ...
                if (!name.isNullOrBlank() && !targetText.isNullOrBlank() && !unit.isNullOrBlank()) {
                    try {
    ...
    private fun showEditHabitDialog(habit: Habit) {
        ...
        // Set selected unit
        val unitIndex = units.indexOf(habit.unit)
        if (unitIndex >= 0) {
        ...
        androidx.appcompat.app.AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(getString(R.string.save)) { _, _ ->
                ...
                val description = etHabitDescription.text?.toString()?.trim() ?: ""
                val targetText = etTargetValue.text?.toString()?.trim()
                val unit = spinnerUnit.selectedItem?.toString() ?: "times"
